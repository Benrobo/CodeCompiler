...
async function init() {
  ...
  textInp.addEventListener("keydown", (e) => {
    if (e.key === "Tab") {
      ...
      textInp.value =
        textInp.value.substring(0, start) + "\t" + textInp.value.substring(end);
  ...
  async function compileJavaCode() {
    ...
    try {
      ...
      let req = await fetch(`${url}/compileJava`, {
        method: "post",
        headers: {
          "content-type": "application/json",
        },
  ...
  async function compileJavaCodeWithInput() {
    ...
    try {
      ...
      outputCode.innerHTML = "Compiling";
      let data = await http
        .post(`${url}/compileJavaInput`, {
          code: codeText,
          ext: "java",
          input: javaInput.value,
        })
        .catch((err) => {
          return err;
        });
      ...
      outputCode.classList.remove("success");
      outputCode.classList.add("error");
      outputCode.innerHTML = "Server Error: Failed to compile code with input";
      loading = false;
    }
  }
}
